...
class SpaceChecker(NodeTask):
    """Verifies the remaining space in a node."""
    def __init__(self, node, minSpace):
        # Node
        super().__init__(node)
        # The minimum space
    ...
    def run(self):
        # Does the node has the required space ?
        if self.node.capacity - self.node.current >= self.minSpace:
            return Task.SUCCES
        else:
            return Task.ECHEC

class Consume(NodeTask):
    """Decreases the incoming edge, increases node and outgoing edge if posible."""
